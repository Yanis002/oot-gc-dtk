/*
    Compile unit: C:\WINDOWS\Profiles\slee\Desktop\ENGLISH ZELDA\Fire\THPPlayer.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x8000F89C -> 0x80010DA8
*/

#include "types.h"

// size = 0x100, address = 0x800EC6F0
static u16 VolumeTable[128];

// size = 0x4, address = 0x80137B18
static s32 Initialized;

// size = 0x40, address = 0x800FBBE0
...
} __anon_0x10501; // size = 0x3C

typedef struct __anon_0x105D9 {
    /* 0x00 */ char magic[4];
    /* 0x04 */ u32 version;
    /* 0x08 */ u32 bufSize;
...
} __anon_0x105D9; // size = 0x30

typedef struct __anon_0x10805 {
    /* 0x0 */ u32 numComponents;
    /* 0x4 */ u8 frameComp[16];
} __anon_0x10805; // size = 0x14

typedef struct __anon_0x1086D {
    /* 0x0 */ u32 xSize;
    /* 0x4 */ u32 ySize;
    /* 0x8 */ u32 videoType;
} __anon_0x1086D; // size = 0xC

typedef struct __anon_0x108ED {
    /* 0x0 */ u32 sndChannels;
    /* 0x4 */ u32 sndFrequency;
    /* 0x8 */ u32 sndNumSamples;
    /* 0xC */ u32 sndNumTracks;
} __anon_0x108ED; // size = 0x10

typedef struct __anon_0x109A7 {
    /* 0x0 */ u8* ytexture;
    /* 0x4 */ u8* utexture;
    /* 0x8 */ u8* vtexture;
    /* 0xC */ s32 frameNumber;
} __anon_0x109A7; // size = 0x10

typedef struct __anon_0x10A5D {
    /* 0x0 */ s16* buffer;
    /* 0x4 */ s16* curPtr;
    /* 0x8 */ u32 validSample;
} __anon_0x10A5D; // size = 0xC

typedef struct __anon_0x10AE7 {
    /* 0x0 */ u8* ptr;
    /* 0x4 */ s32 frameNumber;
    /* 0x8 */ s32 isValid;
} __anon_0x10AE7; // size = 0xC

typedef struct __anon_0x10BD2 {
    /* 0x000 */ struct DVDFileInfo fileInfo;
    /* 0x03C */ struct __anon_0x105D9 header;
    /* 0x06C */ struct __anon_0x10805 compInfo;
    /* 0x080 */ struct __anon_0x1086D videoInfo;
    /* 0x08C */ struct __anon_0x108ED audioInfo;
    /* 0x09C */ void* thpWork;
    /* 0x0A0 */ s32 open;
    /* 0x0A4 */ u8 state;
    ...
    /* 0x0F8 */ struct __anon_0x109A7* dispTextureSet;
    /* 0x0FC */ struct __anon_0x10A5D* playAudioBuffer;
    /* 0x100 */ struct __anon_0x10AE7 readBuffer[10];
    /* 0x178 */ struct __anon_0x109A7 textureSet[3];
    /* 0x1A8 */ struct __anon_0x10A5D audioBuffer[3];
} __anon_0x10BD2; // size = 0x1D0

// size = 0x1D0, address = 0x800FC180
struct __anon_0x10BD2 ActivePlayer;

typedef enum __anon_0x11A4C {
...
typedef enum __anon_0x11B98 {
    VI_XFBMODE_SF = 0,
    VI_XFBMODE_DF = 1,
} __anon_0x11B98;

typedef struct _GXRenderModeObj {
    /* 0x00 */ enum __anon_0x11A4C viTVmode;
    /* 0x04 */ u16 fbWidth;
    /* 0x06 */ u16 efbHeight;
    /* 0x08 */ u16 xfbHeight;
    /* 0x0A */ u16 viXOrigin;
    ...
    /* 0x0E */ u16 viWidth;
    /* 0x10 */ u16 viHeight;
    /* 0x14 */ enum __anon_0x11B98 xFBmode;
    /* 0x18 */ u8 field_rendering;
    /* 0x19 */ u8 aa;
    /* 0x1A */ u8 sample_pattern[12][2];
    /* 0x32 */ u8 vfilter[7];
} __anon_0x11C4B; // size = 0x3C
...
// Erased
static s32 THPPlayerGetAudioInfo(struct __anon_0x108ED* audioInfo) {
    // Parameters
    // struct __anon_0x108ED* audioInfo; // r3
...
// Erased
static s32 THPPlayerGetVideoInfo(struct __anon_0x1086D* videoInfo) {
    // Parameters
    // struct __anon_0x1086D* videoInfo; // r3

    // References
    // -> struct __anon_0x10BD2 ActivePlayer;
}

// Range: 0x80010728 -> 0x800107F4
...
// Range: 0x80010598 -> 0x80010604
static s32 ProperTimingForStart() {
    // References
    // -> struct __anon_0x10BD2 ActivePlayer;
}

// Range: 0x800102FC -> 0x80010598
static void PlayControl(u32 retraceCount) {
    ...
    s32 diff; // r1+0x8
    s32 frameNumber; // r4
    struct __anon_0x109A7* textureSet; // r29
...
// Range: 0x800102A0 -> 0x800102FC
s32 THPPlayerPlay() {
    // References
    // -> struct __anon_0x10BD2 ActivePlayer;
}

// Range: 0x8001002C -> 0x800102A0
...
// Range: 0x8000FF30 -> 0x8000FFFC
static void InitAllMessageQueue() {
    // Local variables
    s32 i; // r28
    struct __anon_0x10AE7* readBuffer; // r3
    struct __anon_0x109A7* textureSet; // r3
    struct __anon_0x10A5D* audioBuffer; // r3

/*
    Compile unit: C:\WINDOWS\Profiles\slee\Desktop\ENGLISH ZELDA\Fire\mcardGCN.c
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8001344C -> 0x8001C450
*/

#include "types.h"

// size = 0xA000, address = 0x800FFE60
static char gMCardCardWorkArea[40960];

// size = 0x4, address = 0x80137B60
s32 currentIdx;

// size = 0x4, address = 0x80137B64
static s32 yes$771;

typedef enum __anon_0x1A683 {
    MC_M_NONE = 0,
    MC_M_LD01 = 1,
    MC_M_LD02 = 2,
    MC_M_LD03 = 3,
...
// size = 0x4, address = 0x80137B68
static enum __anon_0x1A683 prevMenuEntry$772;

// size = 0x4, address = 0x80137B6C
static enum __anon_0x1A683 nextMenuEntry$773;

// size = 0x4, address = 0x80137B70
static s32 toggle2$1029;

// size = 0x4, address = 0x801372B8
static s32 toggle$1034;

// size = 0x4, address = 0x80137B74
...
typedef struct OSCalendarTime {
    ...
    /* 0x20 */ s32 msec;
    /* 0x24 */ s32 usec;
} __anon_0x1AA81; // size = 0x28

// size = 0x28, address = 0x80109E60
struct OSCalendarTime gDate;

// size = 0x4, address = 0x80137B78
s32 bWrite2Card;

// size = 0x28, address = 0x80109E88
s32 bNoWriteInCurrentFrame[10];

typedef struct __anon_0x1ACAD {
...
typedef struct CARDFileInfo {
    ...
    /* 0x10 */ u16 iBlock;
    /* 0x12 */ u16 __padding;
} __anon_0x1AE50; // size = 0x14

typedef struct __anon_0x1AF48 {
    /* 0x000 */ s32 currentGame;
    /* 0x004 */ s32 fileSize;
    /* 0x008 */ char name[33];
    /* 0x02C */ s32 numberOfGames;
    /* 0x030 */ struct __anon_0x1ACAD game;
...
} __anon_0x1AF48; // size = 0x35C

typedef enum __anon_0x1B15E {
    MC_E_NONE = 0,
    MC_E_BUSY = 1,
    MC_E_WRONGDEVICE = 2,
    MC_E_NOCARD = 3,
...
typedef struct _MCARD {
    /* 0x000 */ struct __anon_0x1AF48 file;
    /* 0x35C */ enum __anon_0x1B15E error;
    /* 0x360 */ s32 slot;
...
} __anon_0x1B402; // size = 0x7B8

// size = 0x7B8, address = 0x80109EB0
struct _MCARD mCard;

typedef enum __anon_0x1B8A6 {
    SM_NONE = -1,
    SM_RUNNING = 0,
    SM_STOPPED = 1,
} __anon_0x1B8A6;

typedef struct __anon_0x1B90E {
    /* 0x0 */ s32 nSize;
    /* 0x4 */ s32 nOffsetRAM;
    /* 0x8 */ s32 nOffsetROM;
    /* 0xC */ s32 (* pCallback)();
} __anon_0x1B90E; // size = 0x10

typedef enum __anon_0x1B9BF {
    SRT_NONE = -1,
    SRT_MARIO = 0,
    SRT_WAVERACE = 1,
    SRT_MARIOKART = 2,
...
typedef struct __anon_0x1BC30 {
    /* 0x00 */ void* pFrame;
    /* 0x04 */ void* pSound;
    /* 0x08 */ s32 bException;
    /* 0x0C */ enum __anon_0x1B8A6 eMode;
    /* 0x10 */ struct __anon_0x1B90E romCopy;
    /* 0x20 */ enum __anon_0x1B9BF eTypeROM;
    /* 0x24 */ void* apObject[16];
    /* 0x68 */ u64 nAddressBreak;
    /* 0x70 */ enum __anon_0x1BAF0 storageDevice;
    /* 0x74 */ u8 anException[16];
    /* 0x84 */ s32 bJapaneseVersion;
} __anon_0x1BC30; // size = 0x88

// size = 0x4, address = 0x80137B00
struct __anon_0x1BC30* gpSystem;

typedef enum __anon_0x1BE21 {
    MC_C_NONE = 0,
    MC_C_CONTINUE = 1,
    MC_C_IPL = 2,
    MC_C_GO_TO_GAME = 3,
...
typedef struct __anon_0x1F52A {
    /* 0x0 */ u16 numEntries;
    /* 0x2 */ u8 unpacked;
    /* 0x3 */ u8 pad8;
    /* 0x4 */ enum _GXTlutFmt format;
    /* 0x8 */ char* data;
} __anon_0x1F52A; // size = 0xC

typedef struct __anon_0x1F5F6 {
    /* 0x0 */ struct __anon_0x1F2CF* textureHeader;
    /* 0x4 */ struct __anon_0x1F52A* CLUTHeader;
} __anon_0x1F5F6; // size = 0x8

typedef struct __anon_0x1F667 {
    /* 0x0 */ u32 versionNumber;
    /* 0x4 */ u32 numDescriptors;
    /* 0x8 */ struct __anon_0x1F5F6* descriptorArray;
} __anon_0x1F667; // size = 0xC

// Erased
static void mcardUnpackTexPalette(struct __anon_0x1F667* pal) {
    // Parameters
    // struct __anon_0x1F667* pal; // r1+0x0

    // Local variables
    u16 i; // r4
}

// Range: 0x8001C2AC -> 0x8001C450
...
// Range: 0x800194E4 -> 0x8001967C
s32 mcardReInit(struct _MCARD* pMCard) {
    // Parameters
    // struct _MCARD* pMCard; // r30
}

// Range: 0x80019488 -> 0x800194E4
s32 mcardInit(struct _MCARD* pMCard) {
    // Parameters
    // struct _MCARD* pMCard; // r31
}

// Range: 0x80019064 -> 0x80019488
...
// Range: 0x80017AA0 -> 0x80017C30
s32 mcardFileErase(struct _MCARD* pMCard) {
    // Parameters
    // struct _MCARD* pMCard; // r30
}

// Range: 0x800178F8 -> 0x80017AA0
...
// Range: 0x80017850 -> 0x800178F8
s32 mcardGameRelease(struct _MCARD* pMCard) {
    // Parameters
    // struct _MCARD* pMCard; // r31
}

// Range: 0x80017820 -> 0x80017850
...
// Range: 0x80016E7C -> 0x80017820
s32 mcardMenu(struct _MCARD* pMCard, enum __anon_0x1A683 menuEntry, enum __anon_0x1BE21* pCommand) {
    // Parameters
    // struct _MCARD* pMCard; // r29
    // enum __anon_0x1A683 menuEntry; // r4
    // enum __anon_0x1BE21* pCommand; // r30
...
// Range: 0x80016D9C -> 0x80016E7C
s32 mcardOpenError(struct _MCARD* pMCard, enum __anon_0x1BE21* pCommand) {
    // Parameters
    // struct _MCARD* pMCard; // r3
    // enum __anon_0x1BE21* pCommand; // r4
}

// Range: 0x80016CBC -> 0x80016D9C
s32 mcardOpenDuringGameError(struct _MCARD* pMCard, enum __anon_0x1BE21* pCommand) {
    // Parameters
    // struct _MCARD* pMCard; // r3
    // enum __anon_0x1BE21* pCommand; // r4
}
...
// Erased
static s32 mcardGetError(struct _MCARD* pMCard, enum __anon_0x1B15E* pMCardError) {
    // Parameters
    // struct _MCARD* pMCard; // r1+0x0
    // enum __anon_0x1B15E* pMCardError; // r1+0x4
}
...
s32 mcardOpen(struct _MCARD* pMCard, char* fileName, char* comment, char* icon, char* banner, char* gameName, s32* defaultConfiguration, s32 fileSize, s32 gameSize) {
    ...
    // Local variables
    s32 i; // r19
    enum __anon_0x1BE21 command; // r1+0x34
...
s32 mcardOpenDuringGame(struct _MCARD* pMCard) {
    ...
    // Local variables
    s32 i; // r28
    enum __anon_0x1BE21 command; // r1+0x18
    s32 loadToggle; // r27
...
s32 mcardStore(struct _MCARD* pMCard) {
    ...
    s32 bufferOffset; // r4
    enum __anon_0x1BE21 command; // r1+0x4C8
...
// Range: 0x8001344C -> 0x80013700
s32 mcardUpdate() {
    // Local variables
    s32 j; // r5
    s32 i; // r5
    s32 toggle; // r25
    enum __anon_0x1BE21 command; // r1+0x8
    s32 prevIndex; // r24
    s32 index; // r23
    s32 counter; // r22

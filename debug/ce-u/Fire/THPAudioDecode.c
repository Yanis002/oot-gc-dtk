/*
    Compile unit: C:\WINDOWS\Profiles\slee\Desktop\ENGLISH ZELDA\Fire\THPAudioDecode.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x80010DA8 -> 0x80011144
*/

#include "types.h"

// size = 0x4, address = 0x80137B38
...
} __anon_0x12B16; // size = 0x318

// size = 0x318, address = 0x800FC350
static struct OSThread AudioDecodeThread;

// size = 0x1000, address = 0x800FC668
...
} __anon_0x135CA; // size = 0x3C

typedef struct __anon_0x136A2 {
    /* 0x00 */ char magic[4];
    /* 0x04 */ u32 version;
    /* 0x08 */ u32 bufSize;
...
} __anon_0x136A2; // size = 0x30

typedef struct __anon_0x138CE {
    /* 0x0 */ u32 numComponents;
    /* 0x4 */ u8 frameComp[16];
} __anon_0x138CE; // size = 0x14

typedef struct __anon_0x13936 {
    /* 0x0 */ u32 xSize;
    /* 0x4 */ u32 ySize;
    /* 0x8 */ u32 videoType;
} __anon_0x13936; // size = 0xC

typedef struct __anon_0x139B6 {
    /* 0x0 */ u32 sndChannels;
    /* 0x4 */ u32 sndFrequency;
    /* 0x8 */ u32 sndNumSamples;
    /* 0xC */ u32 sndNumTracks;
} __anon_0x139B6; // size = 0x10

typedef struct __anon_0x13A70 {
    /* 0x0 */ u8* ytexture;
    /* 0x4 */ u8* utexture;
    /* 0x8 */ u8* vtexture;
    /* 0xC */ s32 frameNumber;
} __anon_0x13A70; // size = 0x10

typedef struct __anon_0x13B26 {
    /* 0x0 */ s16* buffer;
    /* 0x4 */ s16* curPtr;
    /* 0x8 */ u32 validSample;
} __anon_0x13B26; // size = 0xC

typedef struct __anon_0x13C16 {
    /* 0x000 */ struct DVDFileInfo fileInfo;
    /* 0x03C */ struct __anon_0x136A2 header;
    /* 0x06C */ struct __anon_0x138CE compInfo;
    /* 0x080 */ struct __anon_0x13936 videoInfo;
    /* 0x08C */ struct __anon_0x139B6 audioInfo;
    /* 0x09C */ void* thpWork;
    /* 0x0A0 */ s32 open;
    /* 0x0A4 */ u8 state;
    ...
    /* 0x0F8 */ struct __anon_0x13A70* dispTextureSet;
    /* 0x0FC */ struct __anon_0x13B26* playAudioBuffer;
    /* 0x100 */ struct __anon_0x1419F readBuffer[10];
    /* 0x178 */ struct __anon_0x13A70 textureSet[3];
    /* 0x1A8 */ struct __anon_0x13B26 audioBuffer[3];
} __anon_0x13C16; // size = 0x1D0

// size = 0x1D0, address = 0x800FC180
struct __anon_0x13C16 ActivePlayer;

typedef struct __anon_0x1419F {
    /* 0x0 */ u8* ptr;
    /* 0x4 */ s32 frameNumber;
    /* 0x8 */ s32 isValid;
} __anon_0x1419F; // size = 0xC

// Range: 0x80011114 -> 0x80011144
...
// Range: 0x80010F94 -> 0x8001106C
static void AudioDecode(struct __anon_0x1419F* readBuffer) {
    // Parameters
    // struct __anon_0x1419F* readBuffer; // r1+0x8

    // Local variables
    struct __anon_0x13B26* audioBuffer; // r30
    u32 i; // r1+0x8
    u32 sample; // r1+0x8
    u32* compSizePtr; // r29
    u8* ptr; // r28
...
// Range: 0x80010EE4 -> 0x80010F94
static void* AudioDecoderForOnMemory(void* ptr) {
    // Parameters
    // void* ptr; // r1+0x8

    // Local variables
    struct __anon_0x1419F readBuffer; // r1+0x10
    s32 tmp; // r4
    s32 size; // r28
    s32 readFrame; // r31
...
// Range: 0x80010EBC -> 0x80010EE4
static void* AudioDecoder() {
    // Local variables
    struct __anon_0x1419F* readBuffer; // r31
}

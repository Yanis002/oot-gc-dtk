/*
    Compile unit: C:\WINDOWS\Profiles\slee\Desktop\ENGLISH ZELDA\Core\xlCoreGCN.c
...
// size = 0x8081, address = 0x800D37A0
u8 gTgPcTPL[32897];

// size = 0x4, address = 0x80137A80
static s32 gnCountArgument;

// size = 0x4, address = 0x80137A84
static char** gaszArgument;

// size = 0x4, address = 0x80137A88
static void* DefaultFifo;

typedef struct __anon_0x243 {
    /* 0x0 */ u8 pad[128];
} __anon_0x243; // size = 0x80

// size = 0x4, address = 0x80137A8C
static struct __anon_0x243* DefaultFifoObj;

typedef enum __anon_0x2AA {
...
typedef enum __anon_0x3F7 {
    VI_XFBMODE_SF = 0,
    VI_XFBMODE_DF = 1,
} __anon_0x3F7;

typedef struct _GXRenderModeObj {
    /* 0x00 */ enum __anon_0x2AA viTVmode;
    /* 0x04 */ u16 fbWidth;
    /* 0x06 */ u16 efbHeight;
    /* 0x08 */ u16 xfbHeight;
    /* 0x0A */ u16 viXOrigin;
    ...
    /* 0x0E */ u16 viWidth;
    /* 0x10 */ u16 viHeight;
    /* 0x14 */ enum __anon_0x3F7 xFBmode;
    /* 0x18 */ u8 field_rendering;
    /* 0x19 */ u8 aa;
    /* 0x1A */ u8 sample_pattern[12][2];
    /* 0x32 */ u8 vfilter[7];
} __anon_0x4AB; // size = 0x3C

// size = 0x3C, address = 0x800F63E0
static struct _GXRenderModeObj rmodeobj;

// size = 0x4, address = 0x80137A90
static void* gpHeap;

// size = 0x4, address = 0x80137A94
static void* gArenaHi;

// size = 0x4, address = 0x80137A98
static void* gArenaLo;

typedef struct _GXTexObj {
    /* 0x0 */ u32 dummy[8];
} __anon_0x774; // size = 0x20

// size = 0x80, address = 0x800F641C
...
typedef struct __anon_0xC5D {
    /* 0x0 */ u16 numEntries;
    /* 0x2 */ u8 unpacked;
    /* 0x3 */ u8 pad8;
    /* 0x4 */ enum _GXTlutFmt format;
    /* 0x8 */ char* data;
} __anon_0xC5D; // size = 0xC

typedef struct __anon_0xD29 {
    /* 0x0 */ struct __anon_0xA02* textureHeader;
    /* 0x4 */ struct __anon_0xC5D* CLUTHeader;
} __anon_0xD29; // size = 0x8
...
// size = 0x3C, address = 0x800F4360
struct _GXRenderModeObj GXNtsc480IntDf;

// size = 0x3C, address = 0x800F439C
struct _GXRenderModeObj GXNtsc480Prog;

// size = 0x3C, address = 0x800F4414
struct _GXRenderModeObj GXPal528IntDf;

// size = 0x3C, address = 0x800F43D8
...
s32 main(s32 nCount, char** aszArgument) {
    ...
    void* pHeap; // r27
    s32 nSizeHeap; // r3
    struct __anon_0xD29* tdp; // r1+0x8

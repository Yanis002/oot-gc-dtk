/*
    Compile unit: C:\WINDOWS\Profiles\slee\Desktop\EUROPE ZELDA\Fire\THPVideoDecode.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x80012DD8 -> 0x800132F8
*/

#include "types.h"

// size = 0x4, address = 0x80181998
...
} __anon_0x1B539; // size = 0x318

// size = 0x318, address = 0x801488F0
static struct OSThread VideoDecodeThread;

// size = 0x1000, address = 0x80148C08
...
} __anon_0x1BFB6; // size = 0x3C

typedef struct __anon_0x1C08E {
    /* 0x00 */ char magic[4];
    /* 0x04 */ u32 version;
    /* 0x08 */ u32 bufSize;
...
} __anon_0x1C08E; // size = 0x30

typedef struct __anon_0x1C2BA {
    /* 0x0 */ u32 numComponents;
    /* 0x4 */ u8 frameComp[16];
} __anon_0x1C2BA; // size = 0x14

typedef struct __anon_0x1C322 {
    /* 0x0 */ u32 xSize;
    /* 0x4 */ u32 ySize;
    /* 0x8 */ u32 videoType;
} __anon_0x1C322; // size = 0xC

typedef struct __anon_0x1C3A2 {
    /* 0x0 */ u32 sndChannels;
    /* 0x4 */ u32 sndFrequency;
    /* 0x8 */ u32 sndNumSamples;
    /* 0xC */ u32 sndNumTracks;
} __anon_0x1C3A2; // size = 0x10

typedef struct __anon_0x1C45C {
    /* 0x0 */ u8* ytexture;
    /* 0x4 */ u8* utexture;
    /* 0x8 */ u8* vtexture;
    /* 0xC */ s32 frameNumber;
} __anon_0x1C45C; // size = 0x10

typedef struct __anon_0x1C512 {
    /* 0x0 */ s16* buffer;
    /* 0x4 */ s16* curPtr;
    /* 0x8 */ u32 validSample;
} __anon_0x1C512; // size = 0xC

typedef struct __anon_0x1C602 {
    /* 0x000 */ struct DVDFileInfo fileInfo;
    /* 0x03C */ struct __anon_0x1C08E header;
    /* 0x06C */ struct __anon_0x1C2BA compInfo;
    /* 0x080 */ struct __anon_0x1C322 videoInfo;
    /* 0x08C */ struct __anon_0x1C3A2 audioInfo;
    /* 0x09C */ void* thpWork;
    /* 0x0A0 */ s32 open;
    /* 0x0A4 */ u8 state;
    ...
    /* 0x0F8 */ struct __anon_0x1C45C* dispTextureSet;
    /* 0x0FC */ struct __anon_0x1C512* playAudioBuffer;
    /* 0x100 */ struct __anon_0x1CC0A readBuffer[10];
    /* 0x178 */ struct __anon_0x1C45C textureSet[3];
    /* 0x1A8 */ struct __anon_0x1C512 audioBuffer[3];
} __anon_0x1C602; // size = 0x1D0

// size = 0x1D0, address = 0x80145F80
struct __anon_0x1C602 ActivePlayer;

typedef struct __anon_0x1CC0A {
    /* 0x0 */ u8* ptr;
    /* 0x4 */ s32 frameNumber;
    /* 0x8 */ s32 isValid;
} __anon_0x1CC0A; // size = 0xC

// Range: 0x800132C8 -> 0x800132F8
...
// Range: 0x80013100 -> 0x80013220
static void VideoDecode(struct __anon_0x1CC0A* readBuffer) {
    // Parameters
    // struct __anon_0x1CC0A* readBuffer; // r24

    // Local variables
    struct __anon_0x1C45C* textureSet; // r28
    u32 i; // r27
    u32* compSizePtr; // r26
    u8* ptr; // r25
    s32 old; // r3
...
// Range: 0x80012FCC -> 0x80013100
static void* VideoDecoderForOnMemory(void* ptr) {
    // Parameters
    // void* ptr; // r1+0x8

    // Local variables
    struct __anon_0x1CC0A readBuffer; // r1+0x10
    s32 old; // r3
    s32 tmp; // r4
    s32 size; // r29
    s32 readFrame; // r28
...
// Range: 0x80012EF0 -> 0x80012FCC
static void* VideoDecoder() {
    // Local variables
    struct __anon_0x1CC0A* readBuffer; // r28
    s32 old; // r3
